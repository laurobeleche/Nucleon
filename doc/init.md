...
All three Linux startup configurations assume the existence of a "neoncore" user
and group.  They must be created before attempting to use these scripts.
The OS X configuration assumes neond will be set up for the current user.
...
setting is not set, neond will shutdown promptly after startup.
...
see `contrib/debian/examples/neon.conf`.
...
Configuration file:  `/etc/neoncore/neon.conf`  
Data directory:      `/var/lib/neond`  
PID file:            `/var/run/neond/neond.pid` (OpenRC and Upstart) or `/var/lib/neond/neond.pid` (systemd)  
Lock file:           `/var/lock/subsys/neond` (CentOS)  

The configuration file, PID directory (if applicable) and data directory
should all be owned by the neoncore user and group.  It is advised for security
reasons to make the configuration file and data directory only readable by the
neoncore user and group.  Access to neon-cli and other neond rpc clients
can then be controlled by group membership.

3b) Mac OS X

Binary:              `/usr/local/bin/neond`  
Configuration file:  `~/Library/Application Support/NucleonCore/neon.conf`  
Data directory:      `~/Library/Application Support/NucleonCore`
Lock file:           `~/Library/Application Support/NucleonCore/.lock`
...
Launch Daemon with a dedicated neoncore user.

...
*Setup instructions for a Gitian build of Nucleon Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nucleon
...
- [Building Nucleon Core](#building-neon-core)
...
Clone the git repositories for Nucleon Core and Gitian.
...
in the Nucleon Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Nucleon Core
----------------

To build Nucleon Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Nucleon Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/neon/.git/
...
and the Nucleon.git repository with the desired tag must both be available locally, and then gbuild must be
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/neon-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/neon-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/neon-osx-build.assert
```
...
[Nucleon.gitian.sigs](https://github.com/MealwormsBiz/gitian.sigs/) repository, or if that's not possible to create a pull
request.

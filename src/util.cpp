    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "neon" is a composite category enabling all Nucleon-related debug output
                if(ptrCategory->count(std::string("neon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NucleonCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NucleonCore
    // Mac: ~/Library/Application Support/NucleonCore
    // Unix: ~/.neoncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NucleonCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NucleonCore";
#else
    // Unix
    return pathRet / ".neoncore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty neon.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override neon.conf
